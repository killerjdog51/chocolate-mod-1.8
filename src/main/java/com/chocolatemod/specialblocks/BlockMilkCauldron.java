...
public class BlockMilkCauldron extends Block{
    ...
    public static final PropertyInteger LEVEL = PropertyInteger.create("stage", 0, 10);
				...
				if (itemstack == null) {
					...
					if (item == Items.sugar) {
						...
						if (meta < 3) {
                            ...
                            {
                                itemstack1 = new ItemStack(Items.sugar, 1, 0);
                            ...
                            this.setWaterLevel(worldIn, pos, state, meta + 1);
						}
					...
					// This too. You didn't include a getItemDamage() condition. That means, it will possible to use any kind of dye (not just cocoa beans). Fixed it.
						...
						if (meta > 4 && meta < 7) {
							...
							worldIn.setBlockState(pos, SpecialBlocks.Cauldron2.getStateFromMeta(1));						
						}
						...
						return true;
						 ...
						 if(meta == 10) {
							if (!playerIn.capabilities.isCreativeMode)
                            {
                                ...
                                ItemStack itemstack2 = new ItemStack(Items.bowl, 1, 0);
						...
						if (meta == 10) {
							ItemStack itemstack2 = new ItemStack(ChocolateItems.bowlLightChocolateMilk, 1, 0);
							if (!playerIn.inventory.addItemStackToInventory(itemstack2)) {
								worldIn.spawnEntityInWorld(new EntityItem(worldIn));
							} else if (playerIn instanceof EntityPlayerMP) {
								((EntityPlayerMP) playerIn).sendContainerToPlayer(playerIn.inventoryContainer);
						...
						if (meta == 0) {
							ItemStack itemstack2 = new ItemStack(Items.milk_bucket, 1, 0);
							if (!playerIn.inventory.addItemStackToInventory(itemstack2)) {
								worldIn.spawnEntityInWorld(new EntityItem(worldIn));
							} else if (playerIn instanceof EntityPlayerMP) {
								((EntityPlayerMP) playerIn).sendContainerToPlayer(playerIn.inventoryContainer);
						...
						if (meta == 0) {
							ItemStack itemstack2 = new ItemStack(ChocolateItems.bowlMilk, 1, 0);
							if (!playerIn.inventory.addItemStackToInventory(itemstack2)) {
								worldIn.spawnEntityInWorld(new EntityItem(worldIn));
							} else if (playerIn instanceof EntityPlayerMP) {
								((EntityPlayerMP) playerIn).sendContainerToPlayer(playerIn.inventoryContainer);
