...
public class BlockLightCauldron extends Block{
	...
	@Override
	 public void setBlockBoundsForItemRender()
	    ...
	    {
	        worldIn.setBlockState(pos, state.withProperty(LEVEL, Integer.valueOf(MathHelper.clamp_int(level, 0, 10))), 2);
	    ...
	    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumFacing side, float hitX, float hitY, float hitZ) {
	    	...
	    	if (worldIn.isRemote) {
				...
				if (itemstack == null) {
					...
					if (item == ChocolateItems.bowlMilk) {
						...
						if (meta == 0) {
							if (!playerIn.capabilities.isCreativeMode)
                            {
							 ...
							 worldIn.setBlockState(pos, state.withProperty(LEVEL, Integer.valueOf(meta = 11)), 2);
						} else if (meta == 11) {
							if (!playerIn.capabilities.isCreativeMode)
                            {
                                itemstack1 = new ItemStack(ChocolateItems.bowlMilk, 1, 0);
							...
							worldIn.setBlockState(pos, SpecialBlocks.Cauldron3.getStateFromMeta(11));						
							...
							worldIn.setBlockState(pos, SpecialBlocks.Cauldron4.getStateFromMeta(1));						
					...
					// This too. You didn't include a getItemDamage() condition. That means, it will possible to use any kind of dye (not just cocoa beans). Fixed it.
					...
					else if (item == ChocolateItems.cocoaPowder) {
						if (meta < 2) {
							if (!playerIn.capabilities.isCreativeMode)
                            {
							...
							worldIn.setBlockState(pos, SpecialBlocks.Cauldron3.getStateFromMeta(6));						
					} else if(meta == 15) {
						if (!playerIn.capabilities.isCreativeMode)
                        {
                            itemstack1 = new ItemStack(ChocolateItems.cocoaPowder, 1, 0);
						...
						worldIn.setBlockState(pos, SpecialBlocks.Cauldron5.getStateFromMeta(1));						
					...
					if (meta == 15) {
						itemstack2 = new ItemStack(ChocolateItems.bowlWhiteChocolateMilk, 1, 0);
						if (!playerIn.inventory.addItemStackToInventory(itemstack2)) {
							worldIn.spawnEntityInWorld(new EntityItem(worldIn));
						} else if (playerIn instanceof EntityPlayerMP) {
							((EntityPlayerMP) playerIn).sendContainerToPlayer(playerIn.inventoryContainer);
