...
public class BowlRawChocolateMilk extends ItemFood {

	public BowlRawChocolateMilk(int par1, float f, boolean par3) {
		super(par1, par3);
	...
	public boolean onItemUse(ItemStack stackIn, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float par8, float par9, float par10) {
		...
		if (!playerIn.canPlayerEdit(pos, side, stackIn)) {
			...
			if (worldIn.getBlockState(pos).getBlock() == Blocks.cauldron && worldIn.getBlockState(pos).getBlock().getMetaFromState(worldIn.getBlockState(pos)) == 0) {
				...
				worldIn.setBlockState(pos, SpecialBlocks.Cauldron6.getDefaultState());
			}
			return true;
		}
	}
